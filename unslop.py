...
Uses Codex to generate samples, identify repetitive patterns ("slop"),
...
class LiveCodexFeed:
    ...
    def handle(self, packet: dict) -> None:
        ...
        if packet_type == "turn.started":
            self.phase = "thinking"
            self.detail = ""
            self.preview = ""
            return
        if packet_type == "turn.completed":
            self.phase = "finishing"
            self.detail = ""
            return
        if packet_type == "item.completed":
            ...
            if item_type == "agent_message":
                ...
                self.detail = ""
                self._set_preview(item.get("text", ""))
            elif item_type == "reasoning":
                self.phase = "thinking"
                self.detail = ""
            elif item_type == "function_call":
                self.phase = "using tools"
                self.detail = self._tool_detail(item.get("arguments", {}))
        elif packet_type == "exec.command_output_delta":
            self.phase = "running commands"
            self._set_preview(self.preview + packet.get("delta", ""))
...
async def run_codex_with_updates(
    message: str,
    prompt: str,
    cwd: str | None = None,
    ...
    feed = LiveCodexFeed(UI, message)
    await feed.start()
    try:
        result = await codex(prompt, cwd=cwd, config=config, stream_handler=feed.handle)
    except Exception:
...
async def codex(
    ...
    """Run a prompt through `codex exec` and return the response."""
    ...
    if config.effort:
        cmd.extend(["-c", f"model_reasoning_effort={config.effort}"])
    cmd.append(prompt)
    ...
    try:
        ...
        def handle_packet(packet: dict) -> None:
            ...
            packet_type = packet.get("type", "")
            item = packet.get("item", {})
            if packet_type == "item.completed" and isinstance(item, dict):
        ...
        try:
            ...
            pass
        raise RuntimeError(f"codex exec timed out after {config.timeout}s")
...
async def codex_batch(
    ...
    """Run many `codex exec` calls with bounded concurrency."""
    ...
    async def _run(idx: int, prompt: str) -> tuple[int, str]:
        ...
        async with sem:
            ...
            for attempt in range(1, retries + 2):
                try:
                    result = await codex(prompt, cwd=cwd, config=config)
                    if on_success:
...
Output ONLY a valid JSON array of strings. No markdown fences, no commentary."""
    ...
    raw = await run_with_spinner(
        "Asking Codex for a prompt set",
        codex(prompt, config=config),
...
async def step_run_samples(
    ...
    if pending_prompts:
        batch_results = await codex_batch(
            pending_prompts,
            concurrency,
            label="sample",
            config=config,
...
async def step_analyze(out: Path, domain: str, dtype: str, expected_count: int, config: CodexConfig) -> None:
    ext = "*.html" if dtype == "visual" else "*.md"
...
Write your full analysis to ./analysis.md"""
    ...
    log("Codex is reading every sample and screenshot.", indent=1)
    await run_codex_with_updates(
...
async def step_generate_skill(out: Path, domain: str, config: CodexConfig) -> None:
...
async def step_before_after(
    ...
    before = await run_with_spinner(
        ...
        codex(before_prompt, config=config),
    ...
    after = await run_with_spinner(
        ...
        codex(after_prompt, config=config),
...
async def run(args: argparse.Namespace) -> None:
    ...
    if not shutil.which("codex"):
        ...
        print("Install Codex CLI and ensure `codex` is on your PATH.")
        sys.exit(1)
    ...
    # 2 — Run samples
    step += 1
    log_step(step, total, f"Run {len(prompts)} samples through Codex")
    ...
    log("  • Codex: copy skill.md into your project and add it as a skill", indent=1)
    log("  • AGENTS.md: paste the contents into your AGENTS.md", indent=1)
...
examples:
  ...
  python unslop.py --domain "Python tutorials" --count 100 --concurrency 10
    ...
    parser.add_argument(
        ...
        help=f"max parallel Codex calls (default: {DEFAULT_CONCURRENCY})",
    )
    parser.add_argument(
        "--model", type=str, default=None,
        help="Codex model name to pass through (e.g. gpt-5.4, gpt-5.4-mini)",
    )
    parser.add_argument(
        "--effort", choices=["low", "medium", "high", "max"], default=None,
        help="Codex reasoning effort to pass through",
    )
    parser.add_argument(
        "--timeout", type=int, default=DEFAULT_TIMEOUT,
        help=f"seconds to wait per Codex call before failing (default: {DEFAULT_TIMEOUT})",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=f"retries per failed Codex call (default: {DEFAULT_RETRIES})",
    )
    parser.add_argument(
